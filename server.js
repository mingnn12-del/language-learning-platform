...
app.post('/api/auth/register', (req, res) => {
  try {
    ...
    const newUser = { id: generateId(), email, username, password, createdAt: new Date().toISOString() };
...
app.get('/api/words/:language', (req, res) => {
  try {
    const db = readDB();
    const words = db.words[req.params.language] || [];
    res.json(words);
  } catch (error) {
...
app.post('/api/progress', (req, res) => {
  try {
    ...
    if (!progress) {
      progress = { id: generateId(), userId, courseId, completedLessons: [], totalTime: 0, wordsMastered: 0, streak: 1, lastStudied: new Date().toISOString(), dailyGoal: 30, dailyProgress: 0 };
...
app.get('/api/achievements/:userId', (req, res) => {
  try {
    const db = readDB();
    const userAchievements = db.achievements.map((a) => ({ ...a, unlocked: a.current >= a.requirement }));
...
app.post('/api/community/posts', (req, res) => {
  try {
    ...
    const db = readDB();
    const newPost = { id: generateId(), userId, username, content, language, likes: 0, likedByMe: false, comments: [], createdAt: new Date().toISOString() };
...
app.post('/api/community/posts/:postId/comments', (req, res) => {
  try {
    ...
    const post = db.posts.find((p) => p.id === req.params.postId);
    if (!post) return res.status(404).json({ error: 'Post not found' });
    const newComment = { id: generateId(), userId, username, content, createdAt: new Date().toISOString() };
